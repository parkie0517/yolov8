...
def test(args, params, model=None):
    filename = "/home/user/heejun/yolov8/개수작.png"
    ...
    samples = torch.from_numpy(img_rgb).permute(2, 0, 1).unsqueeze(0).float()  # shape: (1, 3, H, W)
    ...
    # NMS
    outputs = util.non_max_suppression(outputs, 0.60, 0.65) # prediction, conf_threshold=0.25, iou_threshold=0.45):

    # 결과 시각화: detections는 배치별 리스트임 (여기서는 배치 크기가 1)
    for output in outputs:
        if output is not None:
            # 각 detection은 [x1, y1, x2, y2, confidence, class] 형태로 가정
            for det in output:
                x1, y1, x2, y2, conf, cls = det.cpu().numpy()
                # 원본 이미지(img)는 BGR 색상 순서이므로, 해당 색상으로 표시
                cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
                # 신뢰도(confidence) 텍스트 표시
                label = f"{conf:.2f}"
                cv2.putText(img, label, (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    

    # 결과 이미지 저장
    output_path = "detections.jpg"  # 원하는 파일 경로와 이름으로 수정
